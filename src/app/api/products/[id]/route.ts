...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  const row = await queryFirst("SELECT p.*, c.slug as cat_slug FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.id=?", params.id) as Record<string, unknown> | undefined;
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await queryFirst("SELECT id FROM products WHERE id=?", params.id) as { id: number } | undefined;
  ...
  if (fields.length) { fields.push("updated_at=datetime('now')"); vals.push(params.id); await execRun(`UPDATE products SET ${fields.join(",")} WHERE id=?`, ...vals); }
  return NextResponse.json({ ok: true });
}

export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  await execRun("DELETE FROM products WHERE id=?", params.id);
  return NextResponse.json({ ok: true });
}
