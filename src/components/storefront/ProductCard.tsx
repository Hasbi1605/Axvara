...
export function ProductCard({ product, index = 0, compact = false }: { product: Product; index?: number; compact?: boolean }) {
  ...
  const handleCheckout = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    // Direct buy — do NOT pollute cart, checkout page will isolate ?buy=slug
    router.push(`/checkout?buy=${product.slug}`);
  };

  if (compact) {
    return (
      <Link href={`/produk/${product.slug}`} className="group block ax-glass rounded-[14px] overflow-hidden hover:-translate-y-0.5 transition-all duration-300 flex flex-col">
        <div className="relative aspect-[4/3] overflow-hidden bg-white/[0.03] m-1 rounded-[10px]">
          ...
          <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-[1.04] transition-transform duration-500" loading="lazy" onError={(e) => { const img = e.currentTarget as HTMLImageElement; img.style.display="none"; const ph=img.nextElementSibling as HTMLElement|null; if(ph) ph.style.display="flex"; }} />
  ...
  return (
    <div
      className="group relative ax-glass rounded-[16px] sm:rounded-[22px] overflow-hidden ax-glass-cyan ax-liquid transition-all duration-300 hover:-translate-y-1 flex flex-col"
    ...
    >
      ...
      <div className="px-2 sm:px-3 pb-2.5 sm:pb-3 pt-1 grid grid-cols-2 gap-1.5 sm:gap-2">
        <button
          onClick={handleAdd}
          className="h-8 sm:h-9 rounded-[10px] sm:rounded-xl bg-white/[0.08] border border-white/10 text-[11px] sm:text-[13px] font-semibold text-white flex items-center justify-center gap-1 sm:gap-1.5 hover:bg-white/12 transition backdrop-blur leading-none"
        >
