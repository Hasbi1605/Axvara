...
export function FulfillmentInventoryPanel({ productId, variantId, mode }: { productId: number; variantId: number; mode: string }) {
  ...
  return <div className="mt-4 rounded-2xl border border-[#00E5FF]/15 bg-[#00E5FF]/[0.035] p-4">
    ...
    {mode === "shared" ? <div className="mt-3"><textarea value={sharedText} onChange={(event) => setSharedText(event.target.value)} rows={3} placeholder="Link, akun bersama, atau instruksi yang dikirim ke setiap pembeli…" className="w-full resize-none rounded-xl border border-white/10 bg-[#080C1E]/65 p-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-[#00E5FF]/40" /><button onClick={() => void saveShared()} disabled={loading || !sharedText.trim()} className="mt-2 inline-flex h-9 items-center gap-2 rounded-xl bg-[#00E5FF] px-4 text-xs font-bold text-[#07101f] transition hover:bg-[#00D0E8] disabled:opacity-40">{loading && <Spinner size={13} />} Simpan pesan bersama</button></div> : <div className="mt-3"><textarea value={inventoryText} onChange={(event) => setInventoryText(event.target.value)} rows={4} placeholder={"Satu akun/key per baris\nemail:password\nLICENSE-KEY"} className="w-full resize-none rounded-xl border border-white/10 bg-[#080C1E]/65 p-3 font-mono text-xs text-white outline-none placeholder:text-white/25 focus:border-[#00E5FF]/40" /><div className="mt-2 flex items-center justify-between gap-3"><p className="text-[10px] text-white/35">Maksimal 100 entri per impor.</p><button onClick={() => void importInventory()} disabled={loading || !inventoryText.trim()} className="inline-flex h-9 shrink-0 items-center gap-2 rounded-xl bg-[#00E5FF] px-4 text-xs font-bold text-[#07101f] transition hover:bg-[#00D0E8] disabled:opacity-40">{loading && <Spinner size={13} />} Impor stok unik</button></div></div>}
  </div>;
}
