  ...
  return (
    <section className="mt-5 space-y-4">
      <header className="flex flex-wrap items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-[#00E5FF]/25 bg-[#00E5FF]/10"><IosIcon name="settings" size={19} tint="#00E5FF" /></span>
        <div className="min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#00E5FF]/70">Sistem</p>
          <h1 className="mt-0.5 text-xl font-bold text-white">Pengaturan Toko</h1>
          <p className="mt-0.5 text-sm text-white/45">Identitas dan kontak ini dipakai konsisten di storefront. Perubahan tidak mengubah data pesanan lama.</p>
        </div>
      </header>

      {error && <div role="alert" className="flex items-start gap-2 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-200"><IosIcon name="info" size={15} tint="#F87171" /><span>{error}</span></div>}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="ax-glass rounded-[22px] p-4 sm:p-6">
          {loading ? <div className="flex min-h-64 items-center justify-center"><Spinner size={26} /></div> : <div className="grid gap-4 sm:grid-cols-2">
            <label className="block text-xs font-semibold text-white/55"><span className="flex items-center gap-1.5"><IosIcon name="globe" size={12} tint="white" /> Nama toko</span>
              <input value={form.name} maxLength={40} onChange={(event) => set("name", event.target.value)} className={inputClass} />
            </label>
            <label className="block text-xs font-semibold text-white/55"><span className="flex items-center gap-1.5"><IosIcon name="whatsapp" size={12} tint="#25D366" /> Nomor WhatsApp admin</span>
            ...
            <label className="block text-xs font-semibold text-white/55 sm:col-span-2"><span className="flex items-center gap-1.5"><IosIcon name="chat" size={12} tint="white" /> Tagline</span>
              <input value={form.tagline} maxLength={160} onChange={(event) => set("tagline", event.target.value)} className={inputClass} />
            </label>
            <label className="block text-xs font-semibold text-white/55"><span className="flex items-center gap-1.5"><IosIcon name="clock" size={12} tint="white" /> Jam layanan</span>
            ...
            <label className="block text-xs font-semibold text-white/55"><span className="flex items-center gap-1.5"><IosIcon name="image" size={12} tint="white" /> Logo gambar (opsional)</span>
            ...
            <label className="block text-xs font-semibold text-white/55 sm:col-span-2"><span className="flex items-center gap-1.5"><IosIcon name="info" size={12} tint="white" /> Keterangan legal di footer</span>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4">
            <p className={`flex items-center gap-1.5 text-xs ${dirty ? "text-[#FFCF55]" : "text-white/35"}`}><IosIcon name={dirty ? "info" : "checked"} size={12} tint={dirty ? "#FFB800" : "#22C55E"} />{dirty ? "Ada perubahan yang belum disimpan." : "Semua perubahan sudah tersimpan."}</p>
            <button type="button" onClick={save} disabled={loading || saving || !dirty} className="inline-flex h-10 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#00E5FF] px-5 text-sm font-bold text-[#07101f] transition hover:bg-[#5cefff] disabled:cursor-not-allowed disabled:opacity-40">
              {saving ? <><Spinner size={15} className="mr-2 border-[#07101f]/20 border-t-[#07101f]" /> Menyimpan…</> : <><IosIcon name="checked" size={14} tint="black" /> Simpan Pengaturan</>}
            </button>
          </div>
        </div>
        ...
        <aside className="ax-glass h-fit rounded-[22px] p-5">
          ...
          <dl className="mt-4 space-y-3 text-xs">
            <div className="flex items-center justify-between gap-4"><dt className="flex items-center gap-1.5 text-white/35"><IosIcon name="whatsapp" size={12} tint="#25D366" /> WhatsApp</dt><dd className="text-right text-white/65">{form.whatsappNumber || "—"}</dd></div>
            <div className="flex items-center justify-between gap-4"><dt className="flex items-center gap-1.5 text-white/35"><IosIcon name="clock" size={12} tint="white" /> Layanan</dt><dd className="text-right text-white/65">{form.supportHours || "—"}</dd></div>
          </dl>
          <p className="mt-5 flex items-start gap-2 rounded-xl border border-[#FFB800]/15 bg-[#FFB800]/[0.06] px-3 py-2.5 text-[11px] leading-5 text-[#FFDA72]/80"><IosIcon name="info" size={13} tint="#FFB800" /><span>Konfigurasi Telegram, WhatsApp Baileys, dan QRIS Hook tetap dikelola di menu operasionalnya—bukan di sini.</span></p>
        </aside>
      </div>
    </section>
  );
