...
export function PaymentReconciliation() {
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-3"><HealthCard title="QRIS dinamis" icon="qr-code" value={healthOk ? "Siap" : "Belum lengkap"} ok={healthOk} /><HealthCard title="Event terakhir" icon="clock" value={data.last_event_at ? formatDate(data.last_event_at) : "Belum ada"} ok={Boolean(data.last_event_at)} /><HealthCard title="Perlu dicek" icon="info" value={String(attention)} ok={attention === 0} /></div>
    {data.health && <section className="rounded-[20px] border border-[#00E5FF]/20 bg-[#00E5FF]/[0.05] p-4 sm:p-5">
      <div className="flex flex-wrap items-start gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#00E5FF]/25 bg-[#00E5FF]/10"><IosIcon name="qr-code" size={17} tint="#00E5FF" /></span>
...
function HealthCard({ title, icon, value, ok }: { title: string; icon: "qr-code" | "clock" | "info"; value: string; ok: boolean }) { return <div className={`rounded-2xl border p-4 ${ok ? "border-emerald-400/15 bg-emerald-400/[0.06]" : "border-red-400/15 bg-red-500/[0.06]"}`}><p className="flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-white/40"><IosIcon name={icon} size={12} tint={ok ? "#22C55E" : "#F87171"} /> {title}</p><p className={`mt-2 text-sm font-semibold ${ok ? "text-emerald-300" : "text-red-300"}`}>{value}</p></div>; }
function EventBadge({ status }: { status: EventRow["status"] }) {
  ...
  const icon: "checked" | "close" | "clock" = status === "matched" ? "checked" : status === "failed" ? "close" : "clock";
  const tint = status === "matched" ? "#22C55E" : status === "failed" ? "#F87171" : "#FFB800";
  const tone =
    status === "matched"
  ...
  return <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${tone}`}><IosIcon name={icon} size={11} tint={tint} alt="" />{label}</span>;
}
