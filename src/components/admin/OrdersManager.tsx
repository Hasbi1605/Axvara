...
export function OrdersManager({ onChanged }: { onChanged?: () => void }) {
  ...
  return <div className="mt-4 space-y-4">
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {[["Total pesanan", stats.total, "text-white", "purchase-order", "white"], ["Pending", stats.pending, "text-[#FFB800]", "clock", "#FFB800"], ["Lunas", stats.paid, "text-emerald-300", "checked", "#22C55E"], ["Omzet", formatRupiah(stats.revenue), "text-white", "wallet", "white"]].map(([label, value, tone, icon, iconTint]) => <div key={String(label)} className="ax-glass rounded-2xl p-4"><p className="flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-white/50"><IosIcon name={icon as "purchase-order" | "clock" | "checked" | "wallet"} size={12} tint={String(iconTint)} /> {label}</p><p className={`mt-1.5 font-display text-xl font-bold sm:text-2xl ${tone}`}>{loading ? "—" : value}</p></div>)}
    </div>

    <section className="ax-glass overflow-hidden rounded-[20px]">
      <header className="border-b border-white/10 p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/5"><IosIcon name="purchase-order" size={17} tint="white" /></span><div className="min-w-0"><h2 className="text-sm font-semibold text-white">Pesanan</h2><p className="mt-0.5 text-[11px] text-white/40">Cari, filter, periksa pembayaran, dan proses fulfillment.</p></div><a href={exportHref} className="ml-auto inline-flex h-9 shrink-0 items-center gap-2 rounded-xl bg-[#00E5FF] px-3.5 text-xs font-bold text-[#07101f] transition hover:bg-[#00D0E8]"><IosIcon name="download" size={13} tint="black" /> Export CSV</a></div>
...
function OrderRow({ order, onDetail, onAction }: { order: Order; onDetail: () => void; onAction: (kind: NonNullable<ActionState>["kind"]) => void }) {
  ...
  return <article className="grid gap-4 px-4 py-4 hover:bg-white/[0.025] sm:px-5 lg:grid-cols-[136px_minmax(0,1fr)_auto] lg:items-center">
    {qris && !order.fileName ? <div className="flex h-[90px] items-center justify-center rounded-xl border border-emerald-400/15 bg-emerald-400/[0.06] px-3 text-center"><div><p className="flex items-center justify-center gap-1.5 text-xs font-semibold text-emerald-300"><IosIcon name="qr-code" size={13} tint="#22C55E" /> QRIS otomatis</p><p className="mt-1 text-[10px] leading-4 text-white/35">Tidak perlu bukti. Menunggu QRIS Hook.</p></div></div> : <ProofThumbnail proof={order.fileName} />}
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><button onClick={onDetail} className="font-mono text-xs font-bold text-[#00E5FF] hover:underline">{order.code}</button><StatusBadge status={order.status} /><ChannelBadge channel={order.salesChannel} /><MethodBadge method={order.method} /></div><p className="mt-2 truncate text-sm font-semibold text-white">{order.name || "Tanpa nama"}</p><p className="mt-0.5 flex items-center gap-1.5 truncate text-xs text-white/40"><IosIcon name={order.salesChannel === "whatsapp" ? "whatsapp" : order.salesChannel === "telegram" ? "telegram-app" : "globe"} size={12} tint={order.salesChannel === "whatsapp" ? "#25D366" : order.salesChannel === "telegram" ? "#229ED9" : "white"} /><span className="truncate">{order.wa || order.email || "Kontak tidak tersedia"}</span></p><p className="mt-1 line-clamp-2 text-xs leading-5 text-white/50">{order.items.map((item) => `${item.name} ×${item.qty}`).join(", ")} · {formatRupiah(order.paymentAmount)}</p>{qris && order.fileName && <p className="mt-1 text-[10px] text-emerald-300/65">Bukti hanya referensi; status pembayaran tetap dari QRIS Hook.</p>}</div>
    <div className="flex flex-wrap gap-2 lg:max-w-[310px] lg:justify-end"><button onClick={onDetail} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.05] px-3 text-xs font-semibold text-white/65 transition hover:bg-white/10 hover:text-white"><IosIcon name="info" size={12} tint="white" /> Detail</button>{order.status === "pending" && <>{qris ? <span className="inline-flex h-9 items-center gap-1.5 rounded-full border border-emerald-400/15 bg-emerald-400/[0.06] px-3 text-xs font-semibold text-emerald-300"><IosIcon name="clock" size={12} tint="#22C55E" /> Menunggu Hook</span> : order.salesChannel === "whatsapp" ? order.proofId && order.proofStatus === "submitted" ? <><button onClick={() => onAction("approve")} className="inline-flex h-9 items-center gap-1.5 rounded-full bg-emerald-500 px-3.5 text-xs font-bold text-white transition hover:bg-emerald-400"><IosIcon name="checked" size={12} tint="black" /> Mutasi cocok</button><button onClick={() => onAction("reject")} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-red-400/25 bg-red-500/10 px-3.5 text-xs font-semibold text-red-200 transition hover:bg-red-500/20"><IosIcon name="close" size={12} tint="#F87171" /> Tolak</button></> : <span className="inline-flex h-9 items-center gap-1.5 rounded-full border border-white/10 px-3 text-xs text-white/45"><IosIcon name="clock" size={12} tint="white" /> Menunggu bukti</span> : <button onClick={() => onAction("paid")} className="inline-flex h-9 items-center gap-1.5 rounded-full bg-emerald-500 px-3.5 text-xs font-bold text-white transition hover:bg-emerald-400"><IosIcon name="checked" size={12} tint="black" /> Konfirmasi lunas</button>}<button onClick={() => onAction("cancel")} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-white/10 px-3 text-xs font-semibold text-white/55 transition hover:bg-white/5 hover:text-white"><IosIcon name="close" size={11} tint="white" /> Batalkan</button></>}</div>
  </article>;
}

function OrderDetail({ order, onClose, onAction }: { order: Order; onClose: () => void; onAction: (kind: NonNullable<ActionState>["kind"]) => void }) {
  const qris = order.method.toLowerCase() === "qris";
  return <AdminDialog title="Detail pesanan" onClose={onClose} icon="purchase-order"><div className="space-y-4"><div className="flex flex-wrap items-center gap-2"><p className="font-mono text-sm font-bold text-[#00E5FF]">{order.code}</p><StatusBadge status={order.status} /></div><div className="mt-2 flex flex-wrap items-center gap-2"><ChannelBadge channel={order.salesChannel} /><MethodBadge method={order.method} /><span className="text-xs text-white/40">{formatDate(order.createdAt)}</span></div><div className="grid grid-cols-2 gap-3 text-sm"><Info label="Pelanggan" value={order.name || "—"} icon="user-manual" /><Info label="WhatsApp" value={order.wa || "—"} icon="whatsapp" iconTint="#25D366" /><Info label="Email" value={order.email || "—"} icon="email" iconTint="#FFB800" /><Info label="Pembayaran" value={`${order.method.toUpperCase()} · ${formatRupiah(order.paymentAmount)}`} icon={qris ? "qr-code" : "wallet"} iconTint={qris ? "#00E5FF" : "#25D366"} /></div><div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><p className="text-xs font-semibold uppercase tracking-wide text-white/40">Item</p>{order.items.map((item, index) => <div key={`${item.name}-${index}`} className="mt-3 flex justify-between gap-4 text-sm"><span className="text-white/70">{item.name} ×{item.qty}</span><span className="font-semibold text-white">{formatRupiah(item.price * item.qty)}</span></div>)}</div>{qris ? <div className="flex items-start gap-2.5 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.06] p-3 text-xs leading-5 text-emerald-200"><IosIcon name="qr-code" size={15} tint="#22C55E" /> <span>QRIS diproses otomatis. Bukti gambar, jika ada, hanya dipakai sebagai referensi.</span></div> : order.fileName ? <ProofThumbnail proof={order.fileName} /> : null}{order.adminNote && <Info label="Catatan admin" value={order.adminNote} />}{order.status === "pending" && <div className="flex flex-wrap justify-end gap-2 border-t border-white/10 pt-4">{!qris && (order.salesChannel !== "whatsapp" ? <button onClick={() => onAction("paid")} className="inline-flex h-10 items-center gap-2 rounded-full bg-emerald-500 px-5 text-sm font-bold text-white transition hover:bg-emerald-400"><IosIcon name="checked" size={14} tint="black" /> Konfirmasi lunas</button> : order.proofStatus === "submitted" && <button onClick={() => onAction("approve")} className="inline-flex h-10 items-center gap-2 rounded-full bg-emerald-500 px-5 text-sm font-bold text-white transition hover:bg-emerald-400"><IosIcon name="checked" size={14} tint="black" /> Mutasi cocok</button>)}<button onClick={() => onAction("cancel")} className="inline-flex h-10 items-center gap-2 rounded-full border border-red-400/25 bg-red-500/10 px-5 text-sm font-semibold text-red-200 transition hover:bg-red-500/20"><IosIcon name="close" size={13} tint="#F87171" /> Batalkan pesanan</button></div>}</div></AdminDialog>;
}
...
function AdminDialog({ title, onClose, children, icon = "info", tone = "cyan" }: { title: string; onClose: () => void; children: React.ReactNode; icon?: "purchase-order" | "checked" | "close" | "info" | "qr-code"; tone?: "cyan" | "green" | "red" }) {
  ...
  const toneClass = tone === "green" ? "border-emerald-400/25 bg-emerald-400/10" : tone === "red" ? "border-red-400/25 bg-red-500/10" : "border-[#00E5FF]/25 bg-[#00E5FF]/10";
  const iconTint = tone === "green" ? "#22C55E" : tone === "red" ? "#F87171" : "#00E5FF";
  return <div className="fixed inset-0 z-[80] isolate flex items-end justify-center overflow-hidden bg-[#040612]/95 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section role="dialog" aria-modal="true" aria-label={title} className="relative z-10 max-h-[94dvh] w-full overflow-y-auto rounded-t-[26px] border border-white/10 bg-[#0B1025] p-5 shadow-[0_30px_100px_rgba(0,0,0,.8)] sm:max-w-xl sm:rounded-[26px] sm:p-6"><header className="mb-5 flex items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border ${toneClass}`}><IosIcon name={icon} size={18} tint={iconTint} /></span><h2 className="min-w-0 flex-1 text-lg font-semibold text-white">{title}</h2><button onClick={onClose} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/10 transition hover:bg-white/15" aria-label="Tutup dialog"><IosIcon name="close" size={14} tint="white" /></button></header>{children}</section></div>;
}

function Info({ label, value, icon, iconTint = "white" }: { label: string; value: string; icon?: "user-manual" | "whatsapp" | "email" | "qr-code" | "wallet"; iconTint?: string }) { return <div className="rounded-xl border border-white/[0.06] bg-white/[0.035] p-3"><p className="flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-white/35">{icon ? <IosIcon name={icon} size={11} tint={iconTint} /> : null}{label}</p><p className="mt-1.5 break-words text-sm text-white/75">{value}</p></div>; }
