...
export type AdminSection = "summary" | "products" | "orders" | "categories" | "payments" | "articles" | "banners" | "subscribers" | "agent";
...
const navigation: [AdminSection, string, IosIconName][] = [
  ...
  ["subscribers", "Pelanggan Email", "news"],
...
export function AdminShell({
  ...
  const sidebar = (
    <aside className={`flex h-full w-[240px] shrink-0 flex-col border-r border-white/10 bg-[#090e25] p-3 transition-[width] duration-200 ${collapsed ? "lg:w-[72px]" : "lg:w-[240px]"}`}>
        ...
        {navigation.map(([id, label, icon]) => {
          ...
          return (
            <button
              ...
              className={`flex h-10 w-full items-center gap-3 rounded-xl px-3 text-sm transition ${active ? "bg-[#00E5FF] font-bold text-[#070a1e]" : "text-white/65 hover:bg-white/[0.08] hover:text-white"}`}
            >
  ...
  return (
    <div className="flex min-h-screen bg-[#070a1e]">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-black/55 lg:hidden" onClick={() => setMobileOpen(false)}>
