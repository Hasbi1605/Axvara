...
| FR-S24 | Kredensial 3 jalur (keputusan owner): isi detail akun dikirim LANGSUNG via WA (outbox durable, kunci `wr-delivery:<code>`, potong 1500 char/bagian) + via email Resend template "Detail Akun Siap" (idempoten `wr-cred-email:<code>`, migrasi 0038 untuk kind/channel; pembeli tanpa email dilewati diam) + tampil di panel web (`/pesanan/[code]` dan hasil `/lacak-pesanan` via `WrCredentialsPanel` prefill WA terverifikasi lookup). Formatter key map normalisasi (akses OTP/URL/username tampil rapi, bukan JSON). Delivery settled bila token terbit; dua-duanya gagal = retry, satu gagal = catat parsial | P0 ✅ live 2026-09-18 |
