...
- **Setup Android:** gunakan URL publik `https://axvara.tech/api/webhook/dana`, samakan field secret aplikasi dengan Pages Secret `DANA_WEBHOOK_SECRET`, aktifkan Notification Access + merchant DANA + QRIS Hook Active, dan matikan Debug Mode agar delivery tidak dilewati. Admin menampilkan URL kanonis, nama header, health, dan event masuk tanpa mengekspos nilai secret.
