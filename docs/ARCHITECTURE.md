...
- Ikon admin memakai `IosIcon` (Icons8 iOS 11 Glyph PNG lokal di `public/icons/ios11/`, 4 ukuran 32/48/64/96, tint via CSS filter/mask) dengan komponen badge bersama `ChannelBadge` (Web/globe, Telegram `#229ED9`, WhatsApp `#25D366`), `StatusBadge` (pending/lunas/dibatalkan/kadaluarsa), dan `MethodBadge` (QRIS/bank/e-wallet) dari `src/components/ui/IosIcon.tsx`; seluruh dialog admin memakai header ikon tone (cyan/hijau/merah), `backdrop-blur`, `max-h dvh`, dan tombol tutup hover tanpa mengubah flow/logic.
