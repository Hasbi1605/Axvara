...
describe("WhatsApp Message Formatting", () => {
  it("formats list products message without categories or prices in enhanced JemStore style", () => {
    const products = [
      { name: "Canva Pro 1 Tahun", whatsappAlias: "CANVA" },
      { name: "ChatGPT Plus 1 Bulan", whatsappAlias: "CHATGPT" },
      { name: "Gemini Advanced 1 Bulan", whatsappAlias: "GEMINI" },
    ];
    const msg = waMsg.listProductsMessage(products);

    expect(msg).toContain("「 *LIST MENU AXVARA* 」");
...
describe("Feature Flags", () => {
  ...
  it("preflight payment detects missing payment methods", async () => {
    const previousEnabled = process.env.DANA_QRIS_ENABLED;
    const previousPayload = process.env.DANA_STATIC_QRIS;
    const previousSecret = process.env.DANA_WEBHOOK_SECRET;
    process.env.DANA_QRIS_ENABLED = "true";
    process.env.DANA_STATIC_QRIS = "configured-for-test";
    process.env.DANA_WEBHOOK_SECRET = "configured-for-test";
    ...
    if (previousEnabled === undefined) delete process.env.DANA_QRIS_ENABLED;
    else process.env.DANA_QRIS_ENABLED = previousEnabled;
    if (previousPayload === undefined) delete process.env.DANA_STATIC_QRIS;
    else process.env.DANA_STATIC_QRIS = previousPayload;
    if (previousSecret === undefined) delete process.env.DANA_WEBHOOK_SECRET;
    else process.env.DANA_WEBHOOK_SECRET = previousSecret;
  });
...
describe("Baileys Gateway Payload Parser (P0.1)", () => {
  it("parses a Baileys group payload correctly", () => {
    const groupFixture = {
    ...
    const parsed = parseWhatsAppPayload(groupFixture);
  ...
  it("parses Baileys group media attachment payload", () => {
    const mediaFixture = {
      sender: "120363024823948293@g.us",
      message: "SEABANK",
      member: "628987654321",
      name: "Siti",
      inboxid: "98234125",
      url: "https://axvara-wa-gateway.example/media/random-token",
      filename: "proof_123.jpg",
      extension: "jpg",
    };

    const parsed = parseWhatsAppPayload(mediaFixture);
    expect(parsed).not.toBeNull();
    expect(parsed?.isGroup).toBe(true);
    expect(parsed?.attachment?.url).toBe("https://axvara-wa-gateway.example/media/random-token");
  ...
  it("parses direct message payload and distinguishes from group", () => {
    ...
    const parsed = parseWhatsAppPayload(directFixture);
    expect(parsed).not.toBeNull();
...
describe("Webhook Authentication & Timing-Safe Check (P0.2)", () => {
  ...
  it("authenticates a compatible payload secret for local fixtures", () => {
    const req = new NextRequest("http://127.0.0.1:3000/api/whatsapp/webhook", {
      method: "POST",
    });
    const auth = authenticateWebhook(req, {
...
describe("SSRF Prevention and Media Magic Bytes (P0.5)", () => {
  it("detects private IP addresses and loopbacks", () => {
    ...
    expect(isPrivateIp("axvara-wa-gateway.example")).toBe(false);
  ...
  it("never forwards the webhook token while downloading gateway media", async () => {
    process.env.WHATSAPP_WEBHOOK_TOKEN = "must-not-leak";
...
describe("Outbound Baileys Reply with inboxId (P0.1)", () => {
  ...
  it("sends inboxId and gateway authentication when replying", async () => {
    ...
    let interceptedHeaders = new Headers();
    ...
    expect(parsed.inboxId).toBe("inbox_999");
    expect(interceptedHeaders.get("x-gateway-token")).toBe("dummy_token");
